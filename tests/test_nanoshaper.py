...
def test_run_nanoshaper():
    nanoshaper_runner = nanoshaper.NanoShaper()
    vertex_file, face_file = nanoshaper_runner(pdb_file, test_dir)
    assert os.path.exists(vertex_file) and os.path.getsize(vertex_file) > 0
    assert os.path.exists(face_file) and os.path.getsize(face_file) > 0
