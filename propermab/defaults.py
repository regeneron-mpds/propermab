...
DEFAULT_BINARY_CONFIG = {
    'hmmer_binary_path': None,
    'nanoshaper_binary_path': None,
    'pdb2pqr_path': None,
    'apbs_binary_path': None,
    'multivalue_binary_path': None,
    'require_bash':False
}
DEFAULT_DATA_CONFIG = {
    'immunebuilder_weights_dir': None,
    'atom_radii_file': None
}
DEFAULT_LIB_CONFIG = {
    'apbs_ld_library_paths': None
}
...
class Config:
    ...
    stores paths to binaries, files etc. that might be needed
    by different tools within propermab.
    
    Can be updated, loaded from file etc
    
    Functions essentially as dictionary, designing as object
    to provide possibility of extensions etc
    
    """
    
    def __init__(self):
        self.config = DEFAULT_BINARY_CONFIG
        self.config.update(DEFAULT_DATA_CONFIG)
        self.config.update(DEFAULT_LIB_CONFIG)
    ...
    def override_defaults(self, new_dict):
        """ change any subset of entries in the system config """
        self.config.update(new_dict)
        return None
    
    def update_from_json(self, json_path):
        """ method to load dict of paths etc 
        
        choice to update defaults or only what's in file
        """
        with open(json_path) as f:
            d = json.load(f)
        self.override_defaults(d)
