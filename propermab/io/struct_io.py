...
def write_to_pdb(coords, filename, b_factor=None):
    """Saves the coordinates into a PDB file for visualization purpose.

    Each set of Cartesian coordinates will be represented by a dummy atom:
        the C carbon in the ALA residue.
    ...
    coords : list of np.ndarray
        Coordinates.
    filename : str
        The PDB file to which the coordinates will be written.
    b_factor : list
        A list of values to be assigned to the b factor field in the PDB file.
    """
    atom_record = 'ATOM'
    atom_num = 0
    atom_type = 'C'
    ...
    if b_factor is not None:
        assert len(coords) == len(b_factor)
    else:
        b_factor = [0.0] * len(coords)

    with open(filename, 'wt') as opf:
        for (x, y, z), bf_value in zip(coords, b_factor):
            atom_num += 1
            opf.write(
                f'{atom_record:6s}{atom_num:5d} {atom_type:^4s}{alt_code:1s}{residue_name:3s}'
                f' {chain_id:1s}{residue_num:4d}{insertion_code:1s}   '
                f'{x:8.3f}{y:8.3f}{z:8.3f}{occ_factor:6.2f}{bf_value:6.2f}'
                f'          {element_symbol:>2s}{charge:2s}\n'
            )
