...
def parse_cmd_args():
    ...
    cmd_parser.add_argument(
        '--features', dest='features', type=str, required=True,
        help='''File containing the features to assign to each voxel.'''
    )
    cmd_parser.add_argument(
    ...
    cmd_parser.add_argument(
        '--augment', dest='augment', action='store_true', default=False,
        help='Whether to augment the feature tensor through rotational symmetries.'
    )
    cmd_parser.add_argument(
    ...
    cmd_parser.add_argument(
        '--mode', dest='mode', type=str, required=False, choices=['b', 't'], default='b',
        help='''Mode (text vs. binary) of the output tensor file.'''
    )

    cmd_args = cmd_parser.parse_args()
    if cmd_args.augment and cmd_args.mode == 't':
        cmd_parser.error('Cannot save tensors of rank > 3 in a text file.')
...
def main():
    ...
    # featurize the voxel grid
    with open(cmd_args.features, 'rt') as in_handle:
        feature_values = [float(line.split(',')[-1]) for line in in_handle]
    ...
    if cmd_args.augment:
        ...
        feature_tensor = surface_voxel_grid.feature_tensor

    # write the feature tensor to disk file
    if cmd_args.mode == 'b':
        filename_extension = '.npy'
    else:
        filename_extension = '.txt'
    grid.save_feature_tensor(
        feature_tensor,
        filename=cmd_args.output_prefix + '_feature_tensor' + filename_extension,
        mode=cmd_args.mode
    )
